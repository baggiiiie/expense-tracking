...
import { resolutionRequestContext } from '../src/mastra/runtime.ts';
...
export async function runCase(name: string, keepWorkspace: boolean): Promise<CaseReport> {
  ...
  const registeredScorers: string[] = [];
  ...
  try {
    ...
    const requestContext = resolutionRequestContext({
      ...
      env: fixture.env,
      verifyCommand: testCase.verify.join(' && '),
    });

    const { gates, workflow, steps } = createResolverScorers(async () => {
      const diff = await fixture.shell(`git diff --no-ext-diff --binary ${fixture.initialSha}..HEAD --`, {
        timeoutMs: 60_000,
      });
    ...
    for (const scorer of [...gates, ...workflow, ...Object.values(steps).flat()]) {
      mastra.addScorer(scorer);
      registeredScorers.push(scorer.id);
    }
    ...
    for (const id of registeredScorers) mastra.removeScorer(id);
